...
// ShellCanvas: each native connector process owns one immutable database route.
// Keep the logical hostname in Pg/MySQL options for TLS identity verification.
static TCP_PROXY: std::sync::OnceLock<(String, u16, u16)> = std::sync::OnceLock::new();

#[doc(hidden)]
pub fn set_tcp_proxy(host: &str, port: u16, local_port: u16) -> io::Result<()> {
    if host.is_empty() || port == 0 || local_port == 0 {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "Invalid TCP proxy endpoint"));
    }
    TCP_PROXY.set((host.trim_matches(&['[', ']'][..]).to_owned(), port, local_port))
        .map_err(|_| io::Error::new(io::ErrorKind::AlreadyExists, "TCP proxy is already configured"))
}
...
pub async fn connect_tcp<Ws: WithSocket>(
    ...
    let (host, port) = if let Some((expected_host, expected_port, local_port)) = TCP_PROXY.get() {
        if host != expected_host || port != *expected_port {
            return Err(io::Error::new(io::ErrorKind::PermissionDenied, "TCP destination differs from approved proxy route").into());
        }
